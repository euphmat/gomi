  ...
  return `
    <div id="${p.elementId}" class="party-card relative flex flex-col bg-gray-800/80 rounded border ${borderClass} p-1 ${p.isDead ? 'opacity-40 grayscale' : 'transition-all cursor-pointer hover:scale-[1.02]'}">
      ...
      <div class="battle-stats-container flex flex-col gap-[1px] text-[9px] text-gray-400 mt-auto leading-tight w-full px-0.5 pb-0.5 ${localStorage.getItem('hideBattleStats') !== 'false' ? 'hidden' : ''}">
        <div class="stat-row-atk flex justify-between items-center ${p._atkBuffTurns > 0 ? 'bg-red-900/40 border-red-500/50' : 'bg-gray-900/40 border-transparent'} border rounded px-1 py-0.5 transition-colors">
          <div class="flex items-center gap-[3px]"><span class="stat-icon-atk material-symbols-outlined ${p._atkBuffTurns > 0 ? 'text-red-400' : 'text-slate-400'}" style="font-size: 10px; font-variation-settings: 'FILL' 1">swords</span><span class="stat-label-atk font-bold tracking-wider ${p._atkBuffTurns > 0 ? 'text-red-400' : ''}">ATK</span></div>
          <span class="stat-val-atk ${p._atkBuffTurns > 0 ? 'text-red-400' : 'text-gray-100'} font-black drop-shadow-md">${formatNumber(p._atkBuffTurns > 0 ? Math.floor(p.stats.atk * (1 + p._atkBuffPercent / 100)) : p.stats.atk)}</span>
        </div>
        ...
        <div class="stat-row-mat flex justify-between items-center ${p._matkBuffTurns > 0 ? 'bg-purple-900/40 border-purple-500/50' : 'bg-gray-900/40 border-transparent'} border rounded px-1 py-0.5 transition-colors">
          <div class="flex items-center gap-[3px]"><span class="stat-icon-mat material-symbols-outlined ${p._matkBuffTurns > 0 ? 'text-purple-400' : 'text-slate-400'}" style="font-size: 10px; font-variation-settings: 'FILL' 1">auto_awesome</span><span class="stat-label-mat font-bold tracking-wider ${p._matkBuffTurns > 0 ? 'text-purple-400' : ''}">MAT</span></div>
          <span class="stat-val-mat ${p._matkBuffTurns > 0 ? 'text-purple-400' : 'text-gray-100'} font-black drop-shadow-md">${formatNumber(p._matkBuffTurns > 0 ? Math.floor(p.stats.matk * (1 + p._matkBuffPercent / 100)) : p.stats.matk)}</span>
        </div>
