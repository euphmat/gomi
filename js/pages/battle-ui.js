  ...
  return `
    <div id="${p.elementId}" class="party-card relative flex flex-col bg-gray-800/80 rounded border ${borderClass} p-1 ${p.isDead ? 'opacity-40 grayscale' : 'transition-all cursor-pointer hover:scale-[1.02]'}">
      ...
      <div class="battle-stats-container flex flex-col gap-[1px] text-[9px] text-gray-400 mt-auto leading-tight w-full px-0.5 pb-0.5 ${localStorage.getItem('hideBattleStats') !== 'false' ? 'hidden' : ''}">
        ...
        <div class="stat-row-mdf flex justify-between items-center ${p._mdefBuffTurns > 0 && p._passiveMdefBuffAmount > 0 ? 'bg-indigo-800/60 border-indigo-400 shadow-[0_0_5px_rgba(129,140,248,0.4)]' : p._mdefBuffTurns > 0 || p._passiveMdefBuffAmount > 0 ? 'bg-indigo-900/40 border-indigo-500/50 shadow-none' : 'bg-gray-900/40 border-transparent shadow-none'} border rounded px-1 py-0.5 transition-colors">
