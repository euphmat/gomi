    ...
    header.innerHTML = monsterDefs.map(m => {
      ...
      return `
        <div class="sub-tab-item flex-1 flex items-center justify-center gap-1 px-1.5 py-1 rounded-full cursor-pointer border ${bgClass} ${shadowClass} ${opacity} transition-all min-w-0 mb-1 backdrop-blur-sm" data-id="${m.id}" title="${m.name}">
          <img src="${m.image}" class="w-3.5 h-3.5 shrink-0 object-contain" onerror="this.style.display='none'">
          <span class="text-[10px] font-bold truncate tracking-wide ${isSelected ? 'text-blue-100 drop-shadow-[0_0_5px_rgba(96,165,250,0.8)]' : 'text-slate-400 drop-shadow-md'}">${m.name}</span>
        </div>
      `;
    }).join('');
