...
export async function renderBattleMedalTab(tabContent, targetEntity, playerMedals, currentGold, onMedalUpdated) {
  ...
  if (!isMaxRank && nextRank) {
    ...
    materialRequirements.forEach(({ mat, itemId, owned, required, sufficient }) => {
      ...
      row.id = `battle-medal-mat-row-${itemId}`;
      ...
      row.innerHTML = `
        ...
        <div class="flex items-center gap-0.5 shrink-0">
          <span id="battle-medal-mat-owned-${itemId}" class="text-[11px] font-black ${sufficient ? 'text-emerald-400' : 'text-red-400'}">${formatNumber(owned)}</span>
