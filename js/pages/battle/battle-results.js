...
export const resultMethods = {
  ...
  async saveDeferredData() {
    ...
    const inventoryChanged = Object.keys(this._pendingItemDrops || {}).length > 0;
    const medalResourcesChanged = this.currentGold !== undefined || inventoryChanged;
    ...
    // Keep the Pet DOM stable across auto-battle rounds. Only its material
    // counts and controls change when rewards are persisted.
    if (inventoryChanged && this.currentTab === 'pet') {
      const refreshed = await this.refreshPetTabResources?.();
      if (!refreshed) this.elements?.tabContent?.removeAttribute('data-rendered-tab');
    } else if (medalResourcesChanged && this.currentTab === 'medal') {
